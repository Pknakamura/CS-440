...
def create_linear_data(num_samples, slope, intercept, x_range=[-1.0, 1.0], noise=0.1):
    scaler = abs(x_range[0] - x_range[1])
    offset = ((x_range[0] + x_range[1]) - scaler) / 2
    x = np.random.rand(num_samples, 1) * scaler + offset

    y_noise = np.random.rand(num_samples, 1) * 2 * noise - noise
    y = x * slope + intercept + y_noise
...
def stochastic_gradient_descent(get_batch_gradient, A_init, learning_rate, num_epochs, data_size, batch_size):
    ...
    for epoch in range(num_epochs):
        shuffled = np.random.permutation(data_size)

        for i in range(0, data_size, batch_size):
            A = A - learning_rate * get_batch_gradient(A, shuffled[i : i + batch_size])
...
def ik_loss(arm : Arm, config, goal):
    ...
    return ((goal[0] - end[0])**2 + (goal[1] - end[1])**2)**(1/2)
...
def sample_near(num_samples, config, epsilon=0.1):
    to_return = np.tile(config, (num_samples, 1))
    noise = np.random.rand(num_samples, len(config)) * 2 * epsilon - epsilon
...
def estimate_ik_gradient(loss, config, num_samples):
    samples = sample_near(num_samples, config, 0.1)
    most_loss = np.argmax(np.apply_along_axis(loss, 1, samples))
    diff_vect = samples[most_loss] - config

    return diff_vect / np.linalg.norm(diff_vect)
...
def logistic_error(y_pred, y_true):
    return np.mean(np.apply_along_axis(lambda r : int(int(r[0] > 0.5) - r[1] != 0), 1, np.stack((y_pred, y_true), axis = 1)))
...
def logistic_prediction(x, A, get_modified_features):
    return 1 / (1 + np.exp(-1 * get_modified_features(x) @ A))

# the logistic loss function for binary classification with y_true in {0,1}
#   loss = -1/N * sum_i (y_true[i] * log(y_pred[i]) + (1 - y_true[i]) * log(1 - y_pred[i]))
# y_pred is a numpy array of probabilities of shape (num_samples, 1)
# y_true is a numpy array of 0's and 1's of shape (num_samples, 1)
# NOTES:
...
def get_logistic_regression_features(x):
    ...
    return np.c_[np.sin(x), np.cos(x), np.cos(x[:,0]) * np.sin(x[:,1]), np.sin(x[:,0]) * np.cos(x[:,1]),
                 x[:,0]**2 * x[:,1]**2, x[:,0]**2 * x[:,1], x[:,0] * x[:,1]**2, x[:,0] * x[:,1],
                 x**2, x, x**0]
