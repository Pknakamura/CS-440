...
class NeuralNet(nn.Module):
    ...
    def forward(self, x):
        ...
        reshaped_x = x.view(x.size(dim = 0), 3, 31, 31)
        return self.linear_block(self.cnn_block(reshaped_x).view(-1 , 64 * 7 * 7))
...
def fit(train_set,train_labels,dev_set,epochs,batch_size=100):
    ...
    net = NeuralNet(lrate = 0.1, loss_fn = nn.CrossEntropyLoss(), in_size = train_set.size(dim = 1), out_size = labels.size(dim = 0))
