...
class MazeState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        nbr_states = []
        nbr_pos = self.maze_neighbors(*self.state)
        for new_pos in nbr_pos:
            changed_state = 10 if self.state[2] != new_pos[2] else 0
            new_state = MazeState(
                state = new_pos, 
                goal  = self.goal,
                dist_from_start = self.dist_from_start + ((self.state[0] - new_pos[0])**2 + (self.state[1] - new_pos[1])**2)**0.5 + changed_state,
                maze  = self.maze,
    ...
    def compute_heuristic(self):
        ...
        for goal in self.goal:
            dist = ((self.state[0] - goal[0])**2 + (self.state[1] - goal[1])**2)**0.5
            if dist <= min:
                min = dist
        return min
