...
class Maze:
    ...
    def filter_valid_waypoints(self):
        ...
        """
        valid_waypoints = {i: [w for w in self.__waypoints if not does_alien_touch_wall(self.create_new_alien(w[0],w[1],i), self.walls)] for i in range(len(self.alien.get_shapes()))}
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        """
        sorted_waypoints = self.__valid_waypoints[cur_shape]
        sorted_waypoints.sort(key = lambda x:((cur_waypoint[0] - x[0])**2 + (cur_waypoint[1] - x[1])**2)**0.5)
        if sorted_waypoints[0] == cur_waypoint:
            sorted_waypoints = sorted_waypoints[1:]
        nearest_waypoints = []
        for waypoint in sorted_waypoints:
            if not does_alien_path_touch_wall(self.create_new_alien(*cur_waypoint,cur_shape), self.walls, waypoint):
                nearest_waypoints.append(waypoint)
                if len(nearest_waypoints) >= self.k:
    ...
    def is_valid_move(self, start, end):
        ...
        if start[2] == end[2]:
            return not does_alien_path_touch_wall(self.create_new_alien(*start), self.walls, (end[0],end[1]))
        elif (start[0],start[1]) == (end[0],end[1]):
           return end[2] in range(3) and not does_alien_touch_wall(self.create_new_alien(*end), self.walls)
        return False
