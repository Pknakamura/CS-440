...
class EightPuzzleState(AbstractState):
    def __init__(self, state, goal, dist_from_start, use_heuristic, zero_loc, goal_dict = None):
        '''
    ...
    def get_neighbors(self):
        ...
        if(self.zero_loc[0] != 2):
            ...
            new_state[self.zero_loc[0]][self.zero_loc[1]] = new_state[self.zero_loc[0] + 1][self.zero_loc[1]]
            new_state[self.zero_loc[0] + 1][self.zero_loc[1]] = 0

            nbr_states.append(EightPuzzleState(state = new_state,
                                               goal = self.goal,
        ...
        if(self.zero_loc[1] != 0):
            ...
            new_state[self.zero_loc[0]][self.zero_loc[1]] = new_state[self.zero_loc[0]][self.zero_loc[1] - 1]
            new_state[self.zero_loc[0]][self.zero_loc[1] - 1] = 0
    ...
    def compute_heuristic(self):
        ...
        for y in range(len(self.state)):
            for x in range(len(self.state[y])):
                if self.state[y][x] != 0:
                    total += manhattan(self.goal_dict[self.state[y][x]],(x,y))
