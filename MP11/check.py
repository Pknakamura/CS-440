...
N_re = np.reshape(N, newshape = (-1,4))
true_N_re = np.reshape(true_N, newshape = (-1,4))

for n, true in zip(N_re, true_N_re):
    if n.any() or true.any():
        print(f"{n}  |  {true}")

print("----------------")

N_re = np.reshape(Q, newshape = (-1,4))
true_N_re = np.reshape(true_Q, newshape = (-1,4))
