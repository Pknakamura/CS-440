...
class MultiGoalGridState(AbstractState):
    ...
    def __init__(self, state, goal, dist_from_start, use_heuristic, maze_neighbors, mst_cache):
        ...
        if not mst_cache:
            self.mst_cache[goal] = compute_mst_cost(goal, manhattan)
    ...
    def get_neighbors(self):
        ...
        # -------------------------------
        for l in neighboring_locs:
            new_goals = tuple((g for g in self.goal if g != l))
            if new_goals not in self.mst_cache:
                self.mst_cache[new_goals] = compute_mst_cost(new_goals, manhattan)
            ...
            new_state = MultiGoalGridState(
                ...
                use_heuristic = self.use_heuristic, # stays the same!
                maze_neighbors = self.maze_neighbors, # stays the same!
                mst_cache = self.mst_cache #share cache between all descendants
            )
    ...
    def compute_heuristic(self):
        min = math.inf if len(self.goal) != 0 else 0
        for goal in self.goal:
            dist = manhattan(self.state, goal)
            if dist <= min:
                min = dist

        heuristic = min + self.mst_cache[self.goal]
        if heuristic == 0:
            print(f"Heuristic for state {self.state}, goals {self.goal}: {heuristic + self.dist_from_start}")
        return heuristic
...
class MultiAgentGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for nbr_locs in product(*neighboring_locs):
            ...
            no_collide = len(nbr_locs) == len(tuple(set(nbr_locs)))
            no_cross = True
            for i in range(len(nbr_locs)):
                if no_cross:
                    for other_i in range(len(self.state)):
                        if nbr_locs[i] == self.state[other_i] and other_i != i and nbr_locs[other_i] == self.state[i]:
                            no_cross = False
                            break

            if no_collide and no_cross:
    ...
    def compute_heuristic_admissible(self):
        max = -math.inf
        for agent, goal in zip(self.state, self.goal):
            dist = manhattan(agent, goal)
            if dist > max:
                max = dist
        return max
